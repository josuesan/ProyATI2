...
@app.route('/editar/<ide>', methods = ['POST'])
def edit(ide):
	if ide.isdigit():
		_id = ide
		prod =productos()
		...
		if oneProd == 0:
			...
			oneProd = prod.get_prod(_id)
			print("hola")
			oneProd = prod.set_prod(_id, request.form['nombre'], request.form['precio'], request.form['foto'], request.form['descripcion'], request.form['categoria'], request.form['vendido'])
			print("chao")
			#db.session.commit()
			return json.dumps(oneProd)
	respuesta = {'error':True,'mensaje':'Producto no existe'}
